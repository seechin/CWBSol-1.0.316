...
<script>
    ...
    function generate_run_parameters(work, workspace_substr=""){
        ...
        var plist = [ "solute", "solvent", "traj", "np", "ntb", "rc", "nr", "box", "log", "out", "verbo", "debug", "debugxc", "closure", "steprism", "stephi", "temperature", "save", "report", "display", "rdfbins", "rdfgrps", "fmt", "lsa", "xvvextend", "ndiis", "delvv", "errtol", "sd", "enlv", "coulomb", "coul_p", "ignoreram" ];
...
<?php
    ...
    if (!empty($do)){
        if (strcasecmp($do, "save") == 0){          // save settings
            ...
            if (!empty($set->workspace) && !is_dir(getcwd().'/run/'.$set->workspace)){
                ...
                shell_exec("echo \# CWBSolv: ".$exec." > ".$run_stdout);
  ...
  // -p, -s and -f picker
    ...
    echo ('<div class="container">'."\n");
      ...
      echo ('</tr>'."\n");
      echo ('<tr><td>'."\n");
        echo ('<input type="text" id="solvent" name="solvent" value="" style="width:100%" />');
        echo ('</td><td>'."\n");
        echo ('<input type="text" id="solute" name="solute" value="" style="width:100%" />');
        echo ('</td><td>'."\n");
        echo ('<input type="text" id="traj" name="traj" value="" style="width:100%" />');
      echo ('</td></tr>'."\n");
      echo ('<tr><td>'."\n");
  ...
  // basic options
    ...
    echo ('<div class="itembox"><table>'."\n");
        ...
        echo ('<tr><td><a href="/help.php#iet_debug_level">debug level</a></td><td>');
          ...
          echo ('  <option value=0 '.($set->debug%16==0?"selected=\"selected\"":"").'> level 0</option>');
          echo ('  <option value=1 '.($set->debug%16==1?"selected=\"selected\"":"").'> level 1 (brief)</option>');
          echo ('  <option value=2 '.($set->debug%16==2?"selected=\"selected\"":"").'> level 2 (full)</option>');
          echo ('  <option value=3 '.($set->debug%16==3?"selected=\"selected\"":"").'> level 3 (extra)</option>');
          echo ('</select>'."\n");
  ...
  // output
    ...
    echo ('<div class="itembox"><table>'."\n");
        ...
        echo ('<tr><td>report</td><td>'); echo ('<input type="text" id="report" name="report" value='.$set->report.' hidden />'."\n");
          ...
          echo ('<br>');
          echo ('<input type="checkbox" name="report_ex" id="report_ex" '.($set->report&32?"checked":"").' onClick="checkbox_xor(\'report_ex\',\'report\',32)">excess_chem_potential </input>');
          echo ('<br>');
          echo ('<input type="checkbox" name="report_GF" id="report_GF" '.($set->report&64?"checked":"").' onClick="checkbox_xor(\'report_GF\',\'report\',64)">excess_chem_GF </input>');
          echo ('<br>');
  ...
  // perform calculation
    ...
    $n_running_rismhi3d = shell_exec("pgrep -u `whoami` rismhi3d | wc -l");
    ...
    echo ("<div class=\"container\">\n");
        ...
        echo ('<input type="submit" value="perform calculation" onclick="submit_run(\'calc\', \'Do you want to perform calculation?\n\nSettings will be saved as well.\')" '.($n_running_rismhi3d>0?'disabled':'').'/>'."\n");
        echo ('<input type="submit" value="view screen output" onclick=show_run_log() />'."\n");
        echo ('<input type="submit" '.($n_running_rismhi3d>0?'style="color:red"':'').' value="terminate all calculations" onclick="submit(\'term\', \'Do you want to terminate all calculations?\')" '.($n_running_rismhi3d>0?'':'disabled').'/>'."\n");
    echo ("</div>\n");
