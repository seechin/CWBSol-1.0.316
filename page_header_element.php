<?php
  ...
  echo ('<body>'."\n");
    ...
    if (!empty($theme_banner)||!empty($theme_bbanner)){
        echo('<center><div class="logobanner"'.(empty($theme_bbanner)?'':' style="background-image:url(/images/'.$theme_bbanner.')"').'><div class="container">'.(empty($theme_banner)?'':'<a href="/"><img src="/images/'.$theme_banner.'" class="logo" /></a>').'</div></div></center>'."\n");
    }
    echo ('<div class="menubar"><div class="container">'."\n");
    if (strcasecmp($title, "Clear Water Bay Solvation")==0){
        echo ('  <a href="/"><div class="navba">Home</div></a>'."\n");
    } else {
        echo ('  <a href="/"><div class="navbb">Home</div></a>'."\n");
    }
    if (empty($iet_bin)){
    ...
    //echo ('  <a href="/login.php"><div class="navba">login</div></a>'."\n");
    //echo ('  </div>'."\n");
    echo ('</div></div>'."\n");
