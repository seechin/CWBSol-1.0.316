  ...
  URL:      '. (empty($path_in_http)?(getcwd()==$url?'/':'<font color=#AA0000>(not available)</a>') : ('/'.$path_in_http)) .'
            ...
            if ($filetype==1 || $filetype==-1){  // text files
                ...
                if (substr($url, 0, strlen($pwd)) == $pwd){
                    echo ('<center><img src="'.substr($url, strlen($pwd), strlen($url)-strlen($pwd)).'" id="image" name="image" style="max-width:100%;max-height:100%" /></center>'."\n");
                ...
                if ($original){
                    ...
                    if (file_exists($url_png) && filemtime($url_png) >= filemtime($url)){
                        $pwd = getcwd();
                        echo ('<center><img src="'.substr($url_png, strlen($pwd), strlen($url_png)-strlen($pwd)).'" style="max-width:100%;max-height:100%"/></center>'."\n");
